"""
Build script for Windows ARM64 architecture.
Similar to build-all-win.py but specifically for ARM64 builds.

This script builds all IfcOpenShell packages for Windows ARM64 during CI,
using cmd scripts, and archives them to '~/outputs'.
"""
...
ZIP_TEMPLATE = f"{{package_name}}-v{VERSION}-{SHA}-win-arm64.zip"
...
def build() -> None:
    for python_version in PYTHON_VERSIONS:
        ...
        subprocess.run(
            [str(REPO_WIN / "build-deps.cmd"), "vs2022-ARM64", "Release"],
            check=True,
            text=True,
            input="y\n",
        )
        ...
        run([str(REPO_WIN / "install-ifcopenshell.bat"), "vs2022-ARM64", "Release"])


def archive_executables() -> None:
    # Typically '_installed-vs2022-ARM64'.
