...
class Alignment:
    ...
    def remove_horizontal_layout(cls, alignment: "ifcopenshell.entity_instance") -> None:
        ...
        Key-point referents are removed too, for the same reason stationing
        referents are -- their placements reference the same now-gone basis
        curve. Drawing the alignment again won't bring them back; they have
        to be regenerated via Generate Key Points.
        """
        file = tool.Ifc.get()
        h_layout = ifcopenshell.api.alignment.get_horizontal_layout(alignment)
        if not h_layout:
        ...
        cls.remove_key_point_referents(alignment)
    ...
    @classmethod
    def get_key_point_nests(cls, alignment: "ifcopenshell.entity_instance") -> list:
        """Every IfcRelNests holding key-point referents (P.O.B., P.C., P.V.C., ...) for
        ``alignment``'s top-level alignment and its child alignments.

        Whether an alignment "has key points turned on" is derived from this, never from a
        Blender-side flag: a flag would be missing for a file authored elsewhere (or saved before
        this session), whereas the IFC nesting is always there to read. A key-point nest is one
        whose related objects are all POSITION IfcReferents that don't position any product --
        which excludes the stationing nest (STATION referents), the layout nest (layouts), and
        add_positioning_referent's referents (linked via IfcRelPositions, not nested). Child
        alignments are included because update_key_point_referents' default nests a child
        vertical's/cant's key points on that child rather than on the parent.
        """
        top = cls._get_top_level_alignment(alignment)
        nests = []
        for owner in [top] + cls.get_child_alignments(top):
            for nest in owner.IsNestedBy or []:
                objects = nest.RelatedObjects
                if objects and all(
                    o.is_a("IfcReferent") and o.PredefinedType == "POSITION" and not o.Positions for o in objects
                ):
                    nests.append(nest)
        return nests

    @classmethod
    def has_key_point_referents(cls, alignment: "ifcopenshell.entity_instance") -> bool:
        return bool(cls.get_key_point_nests(alignment))

    @classmethod
    def remove_key_point_referents(cls, alignment: "ifcopenshell.entity_instance") -> None:
        """Delete every key-point referent (and its Blender object) and their nests."""
        from ifcopenshell.api.alignment.update_key_point_referents import _remove_referent

        file = tool.Ifc.get()
        for nest in cls.get_key_point_nests(alignment):
            for referent in list(nest.RelatedObjects):
                referent_obj = tool.Ifc.get_object(referent)
                if referent_obj:
                    bpy.data.objects.remove(referent_obj, do_unlink=True)
    ...
    @classmethod
    def generate_key_point_referents(cls, alignment: "ifcopenshell.entity_instance") -> int:
        """(Re)generate key-point referents for every layout of ``alignment`` -- horizontal,
        every vertical, every cant -- into one IfcRelNests on the top-level alignment, replacing
        any existing ones (including ones loaded from a file), and create a Blender object for
        each. Every layout's key points are named after the top-level alignment.

        Returns the number of referents created; 0 (and nothing left behind) if there's no real
        horizontal geometry to place them on.
        """
        file = tool.Ifc.get()
        top = cls._get_top_level_alignment(alignment)
        cls.remove_key_point_referents(top)

        h_layout = ifcopenshell.api.alignment.get_horizontal_layout(top)
        if not h_layout or not cls.get_real_layout_segments(h_layout):
            return 0

        nest = ifcopenshell.api.alignment.update_key_point_referents(file, h_layout)
        for layout in cls.get_all_vertical_layouts(top) + cls.get_all_cant_layouts(top):
            ifcopenshell.api.alignment.update_key_point_referents(file, layout, rel_nests=nest)

        for referent in nest.RelatedObjects:
            cls.create_object_for_referent(referent)
        return len(nest.RelatedObjects)

    @classmethod
    def update_key_point_referents_if_present(cls, alignment: "ifcopenshell.entity_instance") -> None:
        """Regenerate key-point referents after the alignment's geometry or stationing changed --
        but only if it already has them (see get_key_point_nests); never adds them unasked.

        Every rebuild gives every segment a fresh identity and can move every transition, so
        existing key points would otherwise be left at stale positions with stale stations.
        """
        if cls.has_key_point_referents(alignment):
            cls.generate_key_point_referents(alignment)
