...
class BimTool(WorkSpaceTool):
    ...
    def draw_settings(cls, context, layout, ws_tool):
        ...
        tool_ifc_map = {
            "bim.wall_tool": ["IfcWall"],
            "bim.slab_tool": ["IfcSlab"],
            "bim.door_tool": ["IfcDoor", "IfcDoorStandardCase"],
            "bim.window_tool": ["IfcWindow", "IfcWindowStandardCase"],
            "bim.column_tool": ["IfcColumn"],
            "bim.beam_tool": ["IfcBeam"],
            "bim.duct_tool": ["IfcDuctSegment"],
            "bim.cable_carrier_tool": ["IfcCableCarrierSegment"],
            "bim.pipe_tool": ["IfcPipeSegment"],
            "bim.cable_tool": ["IfcCableSegment"]
        }
        if active_tool.idname in tool_ifc_map:
            ifc_classes = tool_ifc_map[active_tool.idname]
            if context.active_object and len(context.selected_objects) > 0:
                if AuthoringData.data["active_class"] not in ifc_classes:
                ...
                CreateObjectUI.draw(context, layout, ifc_element_type=cls.ifc_element_type)
        elif active_tool.idname == "bim.bim_tool":
...
class CreateObjectUI:
    ...
    def draw_add_object(cls, context):
        ...
        row = cls.layout.row(align=True)
        if AuthoringData.data["ifc_classes"] and cls.props.ifc_class:
            box = cls.layout.box()
            if AuthoringData.data["type_thumbnail"]:
                ...
                for relating_type in AuthoringData.data["paginated_relating_types"]:
                    ...
                    row.operator(
                        ...
                        text="Launch Type Manager" if ui_context != "TOOL_HEADER" else relating_type["name"],
                        emboss=True,
                        )
                    ...
                    if ui_context != "TOOL_HEADER":
                        ...
                        op = row.operator("bim.set_active_type", text=relating_type["name"], emboss=False)
                        op.relating_type = relating_type["id"]

                        row = box.row(align=True)
                        row.alignment = "CENTER"
                        op = row.operator("bim.set_active_type", text=f"{relating_type['ifc_class'].removesuffix('Type')}[{relating_type['predefined_type']}]", emboss=False)
                        op.relating_type = relating_type["id"]
                        ...
                        if relating_type["icon_id"]:
                            box.row().template_icon(icon_value=relating_type["icon_id"], scale=4)
                        else:
                            box.operator("bim.load_type_thumbnails", text="Load Thumbnails", icon="FILE_REFRESH").ifc_class = props.type_class
