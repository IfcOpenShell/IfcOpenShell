...
class ActivateDrawingByAnnotation(bpy.types.Operator, tool.Ifc.Operator):
    ...
    def find_drawing_from_annotation(self, annotation_element):
        ...
        return None
class SelectSimilarTextLiteral(bpy.types.Operator):
    bl_idname = "bim.select_similar_text_literal"
    bl_label = "Select Similar Text Literal"
    bl_description = "Select all text annotations with the same literal value\n\nSHIFT+CLICK to remove from selection"
    bl_options = {"REGISTER", "UNDO"}
    
    literal_value: bpy.props.StringProperty()
    literal_index: bpy.props.IntProperty(default=0)
    attribute_type: bpy.props.StringProperty(default="text")  # "text", "path", "font_size", "newline", "box_alignment"
    ...
    def execute(self, context):
        if not self.literal_value and self.attribute_type in ["text", "path", "box_alignment"]:
            self.report({"WARNING"}, "No value to search for.")
            return {"CANCELLED"}
