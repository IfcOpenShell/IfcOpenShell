    ...
    def draw_item(self, context, layout, data: DocProperties, item: Drawing, icon, active_data, active_propname):
        ...
        if item.is_drawing:
            ...
            if (
                ...
                row.operator("bim.activate_drawing", text="", icon="VIEW_CAMERA", emboss=True, depress=True).drawing = item.ifc_definition_id
            else:
                row.operator("bim.activate_drawing", text="", icon="VIEW_CAMERA_UNSELECTED", emboss=False).drawing = item.ifc_definition_id
        else:
