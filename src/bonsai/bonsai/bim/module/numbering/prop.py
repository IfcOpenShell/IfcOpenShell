...
from bpy.props import IntProperty, StringProperty, EnumProperty, BoolProperty, IntVectorProperty
from bonsai.tool.numbering import Settings, LoadSelection, NumberFormatting, NumberingSystems, SaveNumber, Storeys


class BIMNumberingProperties(PropertyGroup):
    settings_name: StringProperty(
        name="Settings name", description="Name for saving the current settings", default=""
    ...
    numberings_enum = lambda self, initial: [
        (
            "number",
            NumberingSystems.get_numbering_preview("number", initial),
            "Use numbers. Negative numbers are shown with brackets",
        ),
        (
            "number_ext",
            NumberingSystems.get_numbering_preview("number_ext", initial),
            "Use numbers padded with zeroes to a fixed length based on the number of objects selected. Negative numbers are shown with brackets.",
        ),
        (
            "lower_letter",
            NumberingSystems.get_numbering_preview("lower_letter", initial),
            "Use lowercase letters, continuing with aa, ab, ... where negative numbers are shown with brackets.",
        ),
        (
            "upper_letter",
            NumberingSystems.get_numbering_preview("upper_letter", initial),
            "Use uppercase letters, continuing with AA, AB, ... where negative numbers are shown with brackets.",
        ),
    ]
    ...
    custom_storey: EnumProperty(
        ...
        items=lambda self, _: [
            (storey.Name, storey.Name, f"{storey.Name}\nID: {storey.GlobalId}")
            for storey in Storeys.get_storeys(Settings.to_dict(self))
        ],
    ...
    save_type: EnumProperty(
        ...
        items=[
            ("Attribute", "Attribute", "Store number in an attribute of the IFC element"),
    ...
    attribute_name: EnumProperty(
        ...
        items=[
            ("Tag", "Tag", "Store number in IFC Tag attribute"),
    ...
    attribute_name_other: StringProperty(
        name="Other Attribute Name", description="Name of the other attribute to store the number", default="Tag"
    )  # pyright: ignore[reportInvalidTypeForm]
    ...
    pset_name: EnumProperty(
        name="Pset Name", description="Name of the Pset to store the number", items=get_pset_names
    )  # pyright: ignore[reportInvalidTypeForm]

    property_name: StringProperty(
        name="Property Name", description="Name of the property to store the number", default="Number"
    )  # pyright: ignore[reportInvalidTypeForm]

    custom_pset_name: StringProperty(
        name="Custom Pset Name", description="Name of the custom Pset to store the number", default="Pset_Numbering"
    )  # pyright: ignore[reportInvalidTypeForm]
    ...
    if TYPE_CHECKING:
        ...
        initial_element_number: int
        initial_type_number: int
        initial_storey_number: int
        element_numbering: str
        type_numbering: str
        storey_numbering: str
        custom_storey: str
        custom_storey_number: int
        format: str
        save_type: str
        attribute_name: str
        attribute_name_other: str
        pset_name: str
        property_name: str
        custom_pset_name: str
        remove_toggle: bool
