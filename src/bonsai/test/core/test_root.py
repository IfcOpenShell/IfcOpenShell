...
class TestCopyClass:
    ...
    def test_copy_with_new_geometry_derived_from_the_type(self, ifc, collector, root):
        ...
        root.body_representation_is_mapped("original_element").should_be_called().will_return(True)
    ...
    def test_copy_keeps_a_private_body_instead_of_mapping_to_an_unrelated_type_map(
        self, ifc, collector, geometry, root
    ):
        """A type's RepresentationMaps do not have to describe the occurrence's own Body

        (#7991): an occurrence authored with its own private geometry (e.g. IfcFacetedBrep)
        keeps that geometry on duplication even when its type separately carries
        RepresentationMaps, instead of being silently remapped to whatever the type's map
        happens to contain.
        """
        ifc.get_entity("obj").should_be_called().will_return("original_element")
        root.is_element_a("original_element", "IfcRelSpaceBoundary").should_be_called().will_return(False)
        ...
        root.get_element_type("element").should_be_called().will_return("type")
        root.does_type_have_representations("type").should_be_called().will_return(True)
        root.body_representation_is_mapped("original_element").should_be_called().will_return(False)
