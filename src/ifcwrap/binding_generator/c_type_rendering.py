...
def _normalize_cpp_type(cpp_type: str | None) -> str:
    ...
    return " ".join(cpp_type.replace(" &", "&").replace(" *", "*").replace("< ", "<").replace(" >", ">").split())
...
def _render_result_struct_decl(struct: object, spec: BindingIR) -> str:
    ...
    fields = "\n".join(f"    {field.c_type} {field.name};" for field in finalized.fields)
...
def _render_optional_result_struct_decl(type_spec: TypeSpec, spec: BindingIR) -> str:
    ...
    finalized = next(value for value in spec.abi.value_types.values() if value.c_type == c_type)
    fields = "\n".join(f"    {field.c_type} {field.name};" for field in finalized.fields)
