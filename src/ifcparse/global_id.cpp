    ...
    if (zeros != std::string::npos) {
        for (std::string::const_iterator i = string.begin() + zeros; i != string.end(); ++i) {
            ...
            if (character == nullptr) {
                throw ifcopenshell::exception("Failed to decode GlobalId");
            }
...
ifcopenshell::global_id::operator const std::string&() const {
    return string_data_;
}

ifcopenshell::global_id::operator const boost::uuids::uuid&() const {
    return uuid_data_;
}
