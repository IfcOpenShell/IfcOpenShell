...
TEST_CASE("guid_map keeps GlobalIds inline and ignores keys of any other length", "[ifcparse]") {
    int a = 0, b = 0, c = 0;
    ...
    CHECK(map.insert_or_assign(g2, &b));
    CHECK_FALSE(map.insert_or_assign(overlong, &c));
    CHECK_FALSE(map.insert({overlong, &c}).second);
    CHECK(map.size() == 2);
    CHECK(map.find(g1)->second == &a);
    CHECK(map.find(g2)->second == &b);
    CHECK(map.find(overlong) == map.end());
    CHECK(map.count(overlong) == 0);
    ...
    CHECK(visited == 2);
    CHECK(map.insert_or_assign(g1, &c));
    CHECK(map.find(g1)->second == &c);
    CHECK(map.size() == 2);
    CHECK(map.erase(g1) == 1);
    CHECK(map.erase(g1) == 0);
    CHECK(map.erase(overlong) == 0);
    CHECK(map.size() == 1);
...
TEST_CASE("Parallel and paged parsing yield the same instances, attributes, inverses and GlobalIds as serial in-memory parsing", "[ifcparse]") {
