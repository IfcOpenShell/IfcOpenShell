...
template <typename Reader>
character_decoder<Reader>::~character_decoder() {
}
...
namespace {
    ...
    std::string read_string(std::u32string& builder_, Reader& stream_, typename ifcopenshell::character_decoder<Reader>::ConversionMode mode, char substitution_character) {
        ...
        if (mode == ifcopenshell::character_decoder<Reader>::UTF8) {
            ...
            return ifcopenshell::convert_utf8(builder_);
        }
        if (mode == ifcopenshell::character_decoder<Reader>::SUBSTITUTE) {
            std::string result;
        ...
        if (mode == ifcopenshell::character_decoder<Reader>::ESCAPE) {
        ...
        throw ifcopenshell::exception("Invalid conversion mode");
    }
} // namespace
...
character_encoder::character_encoder(const std::string& input)
    : str_(ifcopenshell::convert_utf8(input)) {}

character_encoder::operator std::string() {
...
"{%s}" % ",".join(_())
*/
std::u32string::value_type ifcopenshell::convert_codepage(int codepage, int character) {
    if (codepage < 1 || codepage > 16 || codepage == 12) {
        throw ifcopenshell::exception("Invalid codepage");
    }
    if (character < 0 || character > 255) {
        throw ifcopenshell::exception("Invalid character ordinal");
    }
    ...
    if (result == 0) {
        throw ifcopenshell::exception("Character not defined");
    }
    return result;
}

std::string ifcopenshell::convert_utf8(const std::u32string& string) {
    return boost::locale::conv::utf_to_utf<char>(string);
}

std::u32string ifcopenshell::convert_utf8(const std::string& s) {
    bool is_ascii = true;
    for (char c : s) {
