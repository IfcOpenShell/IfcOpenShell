		...
		namespace kernels {
			namespace opencascade_plugin {
				...
				abstract_kernel* create_kernel(ifcopenshell::file*, ifcopenshell::geom::settings& settings, ifcopenshell::logger& logger) {
					return new ifcopenshell::geom::open_cascade_kernel(settings, logger);
				}
...
extern "C" void emscripten_register_kernel_plugin(ifcopenshell::geom::kernels::kernel_registry* registry) {
	ifcopenshell::geom::kernels::opencascade_plugin::register_plugin(
		*registry,
		ifcopenshell::plugin::module::builtin(ifcopenshell::geom::kernels::opencascade_plugin::plugin_metadata()));
}
#endif
