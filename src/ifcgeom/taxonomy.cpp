...
piecewise_function_evaluator::piecewise_function_evaluator(piecewise_function::const_ptr pwf) : pwf_(pwf) {
}

piecewise_function::const_ptr piecewise_function_evaluator::get_pwf() const {
    return pwf_;
}
...
std::vector<double> piecewise_function_evaluator::evaluation_points() const {
    if (!eval_points_.has_value()) {
        ...
        auto param_type = pwf_->settings_ ? pwf_->settings_->get<ifcopenshell::geometry::settings::PiecewiseStepType>().get() : ifcopenshell::geometry::settings::PiecewiseStepMethod::MAXSTEPSIZE;
        auto param = pwf_->settings_ ? pwf_->settings_->get<ifcopenshell::geometry::settings::PiecewiseStepParam>().get() : 0.5;
...
item::ptr piecewise_function_evaluator::evaluate() const {
    return evaluate(evaluation_points());
}

item::ptr piecewise_function_evaluator::evaluate(double ustart, double uend, unsigned nsteps) const {
...
item::ptr piecewise_function_evaluator::evaluate(const std::vector<double>& dist) const {
...
std::tuple<double, double, const std::function<Eigen::Matrix4d(double u)>*> piecewise_function_evaluator::get_span(double u) const {
    ...
    for (auto& [length, fn] : pwf_->spans()) {
        ...
        auto tolerance = pwf_->settings_ ? pwf_->settings_->get<ifcopenshell::geometry::settings::Precision>().get() : 0.001;
