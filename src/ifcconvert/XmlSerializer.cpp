...
// values are converted. Things like entity instances and lists are omitted.
    ...
    format_attribute(const Argument *argument, IfcUtil::ArgumentType argument_type, const std::string &argument_name) {
    ...
    ptree &descend(IfcObjectDefinition *product, ptree &tree) {
        ...
        if (product->is(Type::IfcSpatialStructureElement)) {
            ...
            IfcObjectDefinition::list::ptr elements = get_related
                    ...
                    (structure, &IfcSpatialStructureElement::ContainsElements,
        ...
        if (product->is(Type::IfcProduct)) {
            std::map<std::string, IfcPresentationLayerAssignment *> layers = IfcGeom::Kernel::get_layers(
                    product->as<IfcProduct>());
            for (std::map<std::string, IfcPresentationLayerAssignment *>::const_iterator it = layers.begin();
                 it != layers.end(); ++it) {
...
void XmlSerializer::finalize() {
    ...
    for (IfcPresentationLayerAssignment::list::it it = layer_assignments->begin();
         it != layer_assignments->end(); ++it) {
    ...
    for (IfcRelAssociatesMaterial::list::it it = materal_associations->begin();
         it != materal_associations->end(); ++it) {
    ...
    boost::property_tree::xml_writer_settings<ptree::key_type> settings = boost::property_tree::xml_writer_make_settings<ptree::key_type>(
            '\t', 1);
#else
