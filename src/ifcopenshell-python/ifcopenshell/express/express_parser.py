...
def _get_cache_key(fn: str) -> bytes:
    """Generate a cache key based on the source file content for integrity verification."""
    with open(fn, "rb") as f:
        content = f.read()
    return hashlib.sha256(content).digest()


def _verify_cache_integrity(cache_file: str, key: bytes) -> bool:
    """Verify the integrity of the cache file using HMAC."""
    ...
    try:
        ...
        expected_sig = hmac.new(key, cache_content, hashlib.sha256).digest()
        return hmac.compare_digest(stored_sig, expected_sig)
    except (OSError, IOError):
        return False


def _write_cache_signature(cache_file: str, key: bytes) -> None:
    """Write HMAC signature for the cache file."""
    sig_file = cache_file + ".sig"
    with open(cache_file, "rb") as f:
        cache_content = f.read()
    sig = hmac.new(key, cache_content, hashlib.sha256).digest()
...
def parse(fn: str) -> mapping.Mapping:
    ...
    cache_key = _get_cache_key(fn)
    # Only load from cache if file exists, is newer than source, and passes integrity check
    if (os.path.exists(cache_file) and
        os.path.getmtime(cache_file) >= os.path.getmtime(fn) and
        _verify_cache_integrity(cache_file, cache_key)):
        with open(cache_file, "rb") as f:
            m = pickle.load(f)
    else:      
        ...
        # Write HMAC signature to verify cache integrity on subsequent loads
        _write_cache_signature(cache_file, cache_key)
    return m
