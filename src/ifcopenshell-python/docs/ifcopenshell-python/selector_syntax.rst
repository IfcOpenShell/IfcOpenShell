...
.. csv-table::
   :header: "Filter", "Type", "Usage", "Example"
    ...
    "Group", "Filter", "``group{{=}}{{value}}``", "``group=Foo`` specifies the criteria that elements must be assigned directly or indirectly (through nested child groups) to an IfcGroup with a ``Name`` attribute with a value of ``Foo``. This filter recursively includes elements from all child groups nested under the matching group."
